...
class PlansController extends Controller
{
    ...
    public function subscribePlan(Request $request){
        ...
        if($request->get('is_trail') == 1)
        {
            ...
            return $this->successResponse(__('msg.subscribed_successfully',[],$this->lang_code));
        }else {
            $plan = Bouquet::find($request->get('plan_id'));
            if (!$plan)
                return $this->errorResponse(__('msg.plan_not_found',[],$this->lang_code),400);
            ...
            $subscribe->vendor_id = vendor()->id;
            $subscribe->bouquet_id = $plan->id;
            $subscribe->price_type = $price_type;
            $subscribe->st_date = $c_date;
            $subscribe->end_date = $subscribe->price_type == 1 ? date('Y-m-d', strtotime($c_date . ' + 1 month')) : date('Y-m-d', strtotime($c_date . ' + 1 years'));
